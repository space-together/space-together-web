...
export const generateMetadata = async (): Promise<Metadata> => {
  return {
    ...
    description: "View learning materials for your child's subject",
  };
};

const ParentMaterialsPage = async (props: {
  ...
  const materials = (materials_res?.data?.data ?? []).filter(
    (m) => m.is_published
  );
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-muted-foreground">
            View learning materials for this subject
          </p>
        </div>
        ...
        <Tabs defaultValue="all" className="w-full">
          ...
          <TabsContent value="lesson-notes" className="space-y-4">
            {filterByType("Lesson Note").length === 0 ? (
              <CommonEmpty
                ...
                description="No lesson notes available yet"
                icon="/icons/book.png"
              />
            ) : (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <TabsContent value="videos" className="space-y-4">
            {filterByType("Video").length === 0 ? (
              <CommonEmpty
                title="No videos"
                description="No videos available yet"
