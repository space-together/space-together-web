...
import { updateClassNameAction, updateClassusernameAction } from "@/services/actions/class-action";
...
export const ClassSettingUsername = ({ getClass }: props) => {
  ...
  const form = useForm<classUpdateUsernameSchemaType>({
    ...
    defaultValues: {
      username: getClass.username ? getClass.username : "",
    },
  });

  const onSubmint = (values: classUpdateUsernameSchemaType) => {
    startTransition(async () => {
      const action = await updateClassusernameAction(getClass.id, values);
